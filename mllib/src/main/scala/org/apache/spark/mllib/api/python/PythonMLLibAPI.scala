...
import org.apache.spark.mllib.fpm.{FPGrowth, FPGrowthModel, PrefixSpan, AssociationRules}
...
private[python] class PythonMLLibAPI extends Serializable {
  ...
  /**
   * Java stub for Python mllib AssociationRules.train().  This stub returns a handle
   * to the Java object instead of the content of the Java object.  Extra care
   * needs to be taken in the Python code to ensure it gets freed on exit; see
   * the Py4J documentation.
   */
  def trainAssociationRules(
      ...
      minConfidence: Double): JavaRDD[AssociationRules.Rule[Any]] = {

    val model = new FPGrowthModel(data)
    new FPGrowthModelWrapper(model).generateAssociationRules(minConfidence)
  }
